...
if __name__ == "__main__":
    ...
    total_grad_c = [[[0]] for s in range(numLayers)]
    for k in range(numTrials):
        ...
        # total_grad_1_c = 0
        # total_grad_2_c = 0
        a = [[0] for i in range(numLayers+1)]
        z = [[0] for i in range(numLayers+1)]

        total_grad_c = [[[0]] for i in range(numLayers)]
        ...
        for test in tests:
            ...
            # # dzLdwL = a[numLayers-2][0]
            # dzLdwL = test_input
            # # print(z[numLayers-2][0])
            # # print(test_input)
            # grad_c = dCdaL * daLdzL * dzLdwL
            # total_grad_c[numLayers-1][0][0] += grad_c
            #
            ...
            # dzLminusonedwLminusone = test_input
            ...
            # dzLdaLminusone = Ws[numLayers - 1][0][0]
            # dCdaLminusone = dCdaL * daLdzL * dzLdaLminusone
            # daLminusonedzLminusone = 1 if z[1][0] >= 0 else q
            # dzLminusonedwLminusone = a[numLayers-3][0]
            # grad_c = dCdaLminusone * daLminusonedzLminusone*dzLminusonedwLminusone
            # total_grad_c[1][0][0] += grad_c
            #
            # dzLminusonedaLminustwo = Ws[numLayers - 2][0][0]
            # dCdaLminustwo = dCdaLminusone * daLminusonedzLminusone * dzLminusonedaLminustwo
            # daLminusonedzLminusone = 1 if z[0][0] >= 0 else q
            # dzLminusonedwLminusone = test_input
            # grad_c = dCdaLminustwo * daLminusonedzLminusone*dzLminusonedwLminusone
            # total_grad_c[0][0][0] += grad_c


            # dCdaL = 2 * difference
            # daLdzL = 1 if z[numLayers-1][0] >= 0 else q
            # dzLdwL = a[numLayers-1][0]
            # # dzLdwL = test_input
            # # print(z[numLayers-2][0])
            # # print(test_input)
            # grad_c = dCdaL * daLdzL * dzLdwL
            ...
            # # dCdaLminusone = dCdaL * daLdzL * dzLdaLminusone
            #
            # dzLdaLminusone = Ws[numLayers - 2][0][0]
            # dCdaLminusone = dCdaL * daLdzL * dzLdaLminusone
            # daLminusonedzLminusone = 1 if z[0][0] >= 0 else q
            # dzLminusonedwLminusone = test_input
            # grad_c = dCdaLminusone * daLminusonedzLminusone*dzLminusonedwLminusone
            ...
            # dCdaLminusone = dCdaL * daLdzL * dzLdaLminusone
            # daLminusonedzLminusone = 1 if z[1][0] >= 0 else q
            # dzLminusonedwLminusone = Ws[1][0][0]
            # grad_c = dCdaLminusone * daLminusonedzLminusone*dzLminusonedwLminusone
            # total_grad_c[1][0][0] += grad_c
            #
            # dzLminusonedaLminustwo = Ws[numLayers - 2][0][0]
            ...
            # casc = Ws[j][0][0]
            # dCdaLminusone = dCdaLminusone * dzLminusonedwLminusone * casc

            # for j in range(numLayers-2, -1, -1):
            #     # dzLdaLminusone = Ws[j+1][0][0]
            #     # dCdaLminusone = dCdaL * daLdzL * dzLdaLminusone
            #     daLminusonedzLminusone = 1 if z[j][0] >= 0 else q
            #     dzLminusonedwLminusone = Ws[j][0][0]
            #     grad_c = dCdaLminusone * daLminusonedzLminusone *dzLminusonedwLminusone
            #     total_grad_c[j][0][0] += grad_c
            #     casc = Ws[j][0][0]
            #     dCdaLminusone = dCdaLminusone * daLminusonedzLminusone * casc
